...
1. `operator()` with accumulator `sum`
...
```c++
Kokkos::View<double*> a("a", 10);
Kokkos::View<double*>::value sum {0.0};                  // <1>
Kokkos::parallel_for(10, MyFunctor(a), sum);             // <2>
```

1. Define and initialize reduction variable
2. Create functor with view `a` and use it directly


## Synchronization

- Execution pattern *may* run asynchrously
- Synchronize using `Kokkos::fence()`
