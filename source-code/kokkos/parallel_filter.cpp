...
int main(int argc, char* argv[]) {
    ...
    Kokkos::initialize(argc, argv);
    {
        ...
        Kokkos::parallel_for("filter", data_dev.extent(0), KOKKOS_LAMBDA(const int i) {
            mask(i) = data_dev(i) > 0.5;
        });
        Kokkos::fence();
        // Perform a parallel scan on the  maks to get the number of elements greater than 0.5
        // to the left of each element, inclusive; the final value of the scan is stored in count
        int count = 0;
        Kokkos::parallel_scan("scan_mask", mask.extent(0), KOKKOS_LAMBDA(const int i, int& update, const bool final) {
            update += mask(i);
            if (final) mask(i) = update;
        }, count);
        Kokkos::fence();
        std::cout << std::endl << "Number of elements greater than 0.5: " << count << std::endl;
        // Create a view to store the filtered data
        Kokkos::View<double*> filtered_data("filtered_data", count);
        // Filter the data
        Kokkos::parallel_for("filter_data", data_dev.extent(0), KOKKOS_LAMBDA(const int i) {
            if (data_dev(i) > 0.5) {
