...
int main(int argc, char* argv[]) {
  Kokkos::initialize(argc, argv);
  {
    ...
    const int nr_iter{argc > 2 ? std::stoi(argv[2]) : 1};
    matrix_view_t A("A", N, N);
    matrix_view_t B("B", N, N);
    Kokkos::parallel_for(
        "init_A_B", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {N, N}),
    ...
    for (int iter = 0; iter < nr_iter; ++iter) {
      Kokkos::parallel_for(
          "gemm_out_loop", policy_t(N, Kokkos::AUTO),
    ...
    Kokkos::parallel_reduce(
        "sum_B", Kokkos::MDRangePolicy<Kokkos::Rank<2>>({0, 0}, {N, N}),
